...
### ✈️ NextTrip - Travel Package Marketplace

A full-stack travel marketplace connecting travelers, vendors, and admins across three dedicated React Native apps.

**Problem Solved**

Travelers lack a unified platform to discover, compare, and book curated travel packages — while vendors have no structured way to list packages, manage bookings, and receive payouts through an admin-governed workflow.

**Solution**

Built a complete 3-app travel marketplace covering traveler discovery and booking, a vendor portal for package management, and an admin portal for platform governance — all backed by a single Node.js/Supabase backend.

**Key Features**

- 🗺️ Package discovery with advanced search across 6 filter dimensions (destination, category, price, duration, rating, amenities)
- 🏢 20-screen vendor portal with full package CRUD, Cloudinary image management, day-level itinerary builder, and multi-tier seasonal pricing
- 🔐 JWT-based auth with database-driven role lookup for 3 user types (traveler, vendor, admin) and Supabase row-level security policies
- 🛠️ 19-screen admin portal with 10 management modules — vendor approval, package moderation, booking management, review curation, category/location CRUD, and payout tracking
- 📋 Append-only audit log with Supabase service-role-only RLS enforcement for compliance tracking
- 🔔 Expo push notifications with device token management for 5 notification types across all user roles
- ⚙️ 95 REST endpoints across 17 backend service modules backed by 22 PostgreSQL tables

**Impact**

- Delivered a production-style multi-role marketplace spanning 59 screens across 3 apps
- Combined vendor onboarding, package lifecycle management, and admin governance into a single cohesive platform

**Tech Used**  
React Native · Expo · Node.js · Express · TypeScript · Supabase · PostgreSQL · Cloudinary · TanStack Query · Zustand
